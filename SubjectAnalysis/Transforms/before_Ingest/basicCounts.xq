...
declare namespace dcadesc="http://nils.lib.tufts.edu/dcadesc/";
...
(: VARIABLES - QUERYING :)
  ...
  declare variable $wwoFiles := collection('files/?select=*.xml');
 
  declare variable $cavendish := doc('files/cavendish.blazing.xml');
  
  declare variable $art := doc('xml/ingestThis.xml');
  
  (: Change this to your XPath query. (Or your XQuery!) :)
    (:  For example: $VARIABLE/XPATH                    :)
  declare variable $query := $art//dcadesc:subject/normalize-space(.);
