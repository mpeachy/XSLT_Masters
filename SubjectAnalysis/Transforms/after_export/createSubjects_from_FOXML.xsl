...
<!-- Vi.*y\) finds Vicenza (Italy) -->
    <xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
        ...
        <xsl:variable name="contribname">
            <xsl:for-each select="//dc:dc/*[namespace-uri()='http://purl.org/d/terms/' and local-name()='contributor']"> 
                <xsl:sort select="."/>
                <xsl:for-each select=".[not(.=following::dc:dc/*[namespace-uri()='http://purl.org/d/terms/' and local-name()='contributor'])]">
                    <xsl:value-of select="replace(.,'(\w$|\W$)','$1|')"/>
                </xsl:for-each>
            </xsl:for-each>
        </xsl:variable>
